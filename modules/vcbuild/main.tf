...
resource "local_file" "vcbuild_output" {
  filename = "C:\\vcenter\\vctemplate.json"
  content  = local.vctemplate_out
}
resource "null_resource" "vc" {
   provisioner "local-exec" {
     command = "C:\\vcenter\\m-vmvcs7\\vcsa-cli-installer\\win32\\vcsa-deploy.exe install --accept-eula --acknowledge-ceip --terse --no-ssl-certificate-verification c:\\vcenter\\vctemplate.json"
   }
 }